...
double EvaluationFunction2(TVector<double> &v, RandomState &rs){
    ...
    for (double t = 0.0; t < 200; t += StepSize) {
        // Step simulation
        for (int n = 43; n<=49; n++){traces >> trace_value; w.n.SetNeuronState(n, trace_value);}
        w.Step(StepSize);
    }
    ...
    for (double t = 200; t <= 220; t += StepSize) {
        // Step simulation
        for (int n = 43; n<=49; n++){traces >> trace_value; w.n.SetNeuronState(n, trace_value);}
        for (int n = 1; n<=7; n++){act << w.n.NeuronState(n) << " ";}  // save recording on head segment
        for (int n = 43; n<=49; n++){act_played << w.n.NeuronState(n) << " ";}  // save played activity in the tail
        act << endl; 
        act_played << endl; 
        w.Step(StepSize);
    }
