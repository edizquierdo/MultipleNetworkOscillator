...
double EvaluationFunction(TVector<double> &v, RandomState &rs, int &ConstantInput){
    ...
    // Intersegment synapse tested
    double SynWeight = phenotype(42);
    phenotype(42) = 0.0; // removed AS -- VA
    ...
    for (int seg = 0; seg < 6; seg++){ // set opposite external input to AS and VAnext
        w.n.SetNeuronExternalInput(AS + seg*7,      SynWeight * 0.75*(ConstantInput%21- 10));
        w.n.SetNeuronExternalInput(VA + (seg+1)*7,  SynWeight * 0.75*(ConstantInput/21- 10));
    }
