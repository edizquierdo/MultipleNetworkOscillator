...
double EvaluationFunction(TVector<double> &v, RandomState &rs, int &ConstantInput){
    ...
    // Intersegment synapse tested
    double SynWeight = phenotype(44 );
    phenotype(44) = 0.0; // removed VB -- DB
    ...
    for (int seg = 0; seg < 6; seg++){ // set opposite external input to VB and DBnext
        w.n.SetNeuronExternalInput(VB + seg*7,      SynWeight * 0.75*(ConstantInput%21- 10));
        w.n.SetNeuronExternalInput(DB + (seg+1)*7,  SynWeight * 0.75*(ConstantInput/21- 10));
    }
