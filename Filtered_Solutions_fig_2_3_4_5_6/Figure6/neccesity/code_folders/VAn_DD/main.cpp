...
double EvaluationFunction(TVector<double> &v, RandomState &rs, int &ConstantInput){
    ...
    // Intersegment synapse tested
    double SynWeight = phenotype(41);
    phenotype(41) = 0.0; // removed VAn -> DD
    ...
    for (int seg = 0; seg < 6; seg++){ // set opposite external input to VB and DBnext
        w.n.SetNeuronExternalInput(DD + seg*7,   SynWeight * (ConstantInput/440));
//        w.n.SetNeuronExternalInput(DD + (seg+1)*7, SynWeight * (ConstantInput/440));
    }
