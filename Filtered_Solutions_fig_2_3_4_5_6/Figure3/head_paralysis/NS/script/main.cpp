...
double EvaluationFunction2(TVector<double> &v, RandomState &rs){
    ...
    w.SetAVB(0.0);
    w.SetAVA(0.0);
//        // Kill Intersegment connections from unit 3 to 4
    int seg = 2;
    w.n.SetChemicalSynapseWeight(DB+seg*7, DD+(seg+1)*7, 0.0);
    w.n.SetChemicalSynapseWeight(VA+(seg+1)*7, DD+seg*7, 0.0);
    w.n.SetElectricalSynapseWeight(AS+seg*7, VA+(seg+1)*7, 0.0);
    w.n.SetElectricalSynapseWeight(DA+seg*7, AS+(seg+1)*7, 0.0);
    w.n.SetElectricalSynapseWeight(VB+seg*7, DB+(seg+1)*7, 0.0);
    
    
    for (double t = 0.0; t < 40; t += StepSize){
        // kill output for cholinergic neurons inthe three more anterior segments
        w.Step(StepSize);
    }
    cout << "finalized adaptation" << endl;

    // Time loop
    for (double t = 0.0; t <= 40; t += StepSize) {
        w.Step(StepSize);
        w.DumpBodyState(body, skip_steps);
        w.DumpCurvature(curv, skip_steps);
    }
    cout << "finish" << endl; 
    body.close();
    curv.close();
