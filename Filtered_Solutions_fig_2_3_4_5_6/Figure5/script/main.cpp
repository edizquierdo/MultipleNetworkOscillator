...
double ASinput, DAinput, DBinput;
// ------------------------------------
...
//////// evaluation ////////////
/////////////////////////////
double Evaluation(TVector<double> &v, RandomState &rs){
    // osc.dat will save the total change in derivative for VA and VB 
    // in three condition: Full VNC unit, without AS_VD and without DA_DD
    double osca = 0.0, oscb = 0.0;
    double VAprev, VBprev;
    ...
    ////////  Full unit simulation /////////////////
    Worm w(phenotype);
    w.InitializeState(rs);

    w.n.SetNeuronExternalInput(AS, ASinput);
    w.n.SetNeuronExternalInput(DA, DAinput);
    w.n.SetNeuronExternalInput(DB, DBinput);
    ...
    for (double t = 0.0; t <= 20; t += StepSize) {
        // Step simulation
        w.Step(StepSize);
        osca += abs(w.n.NeuronOutput(VA) - VAprev);
        oscb += abs(w.n.NeuronOutput(VB) - VBprev);
        VAprev = w.n.NeuronOutput(VA);
        VBprev = w.n.NeuronOutput(VB);
    }
    oscillation << osca/20.0 << " " << oscb/20.0 << " ";
    
    ///////////////////////////////////////////////
    ////////  Full unit without AS_VD simulation /////////////////
    osca = 0.0, oscb = 0.0;
    double AS_VD = phenotype[23];
    w.n.SetChemicalSynapseWeight(AS, VD, 0.0);
    w.InitializeState(rs);
    ...
    ////////  Full unit without DA_DD simulation /////////////////
    osca = 0.0, oscb = 0.0;
    w.n.SetChemicalSynapseWeight(AS, VD, AS_VD);
    w.n.SetChemicalSynapseWeight(DA, DD, 0.0);
    w.InitializeState(rs);
